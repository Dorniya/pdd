...
async function enableSemantics(driver) {
  ...
  const enabled = await driver.executeScript(() => {
    function findSemantics(root) {
      ...
      if (root.tagName === 'FLT-SEMANTICS-PLACEHOLDER') {
        return root;
      }
    ...
    if (view) {
      if (view.shadowRoot) {
        placeholder = view.shadowRoot.querySelector(
  ...
  if (enabled) {
    console.log(
      '[DriverHelper] Flutter accessibility/semantics successfully enabled.'
    );
  } else {
    console.log(
      '[DriverHelper] Flutter semantics placeholder not found. Semantics might be auto-enabled or unavailable.'
    );
  }
...
async function takeScreenshot(driver, testName) {
  try {
    ...
    console.log(
      `[DriverHelper] Screenshot captured on failure: ${filepath}`
    );

    return filepath;
  } catch (error) {
    console.error(
      '[DriverHelper] Failed to capture screenshot:',
      error.message
    );
    return null;
  }
...
async function getBrowserLogs(driver) {
  try {
    return 'Browser logs not available in current Selenium version';
  } catch (error) {
