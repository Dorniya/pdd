...
const fs = require('fs');
const { createDriver, enableSemantics } = require('./utils/driver_helper');
...
function parseTestTitle(title, filePath) {
  ...
  // Default parsing if title doesn't match standard schema
  const typeMap = {
    'e2e': 'End-to-End',
...
async function main() {
  console.log('==================================================');
  console.log('      Starting Yoga App Test Automation Suite     ');
  console.log('==================================================');
  ...
  if (!isUnitOnly) {
    try {
      console.log('[Runner] Launching browser driver...');
      driver = await createDriver();
      global.driver = driver;
      
      console.log(`[Runner] Navigating to target application: ${config.baseUrl}`);
      await driver.get(config.baseUrl);
      
      console.log('[Runner] Injecting and enabling accessibility/semantic layers...');
      await enableSemantics(driver);
    } catch (error) {
      console.error('[Runner] CRITICAL: Failed to initialize browser driver:', error.message);
      process.exit(1);
    }
  }
  ...
  mocha.rootHooks({
    async afterEach() {
      if (this.currentTest && this.currentTest.state === 'failed' && global.driver) {
        try {
          const { takeScreenshot, getBrowserLogs } = require('./utils/driver_helper');
          const testName = this.currentTest.fullTitle();
          
          console.log(`[RootHooks] Test failed: "${testName}". Capturing diagnostics...`);
          
          this.currentTest.screenshotPath = await takeScreenshot(global.driver, testName);
          this.currentTest.browserLogs = await getBrowserLogs(global.driver);
        } catch (e) {
  ...
  // Find all test files matching *.test.js in subdirectories
  const searchDirs = ['e2e', 'functional', 'validation', 'unit'];
  const testFiles = [];

  searchDirs.forEach(dir => {
    // If running unit tests only, skip web-based directories
    if (isUnitOnly && dir !== 'unit') return;
  ...
  const runner = mocha.run(async (failures) => {
    ...
    // Shut down driver if running web tests
    if (driver) {
      console.log('[Runner] Shutting down browser driver...');
      try {
        await driver.quit();
      } catch (err) {
        console.error('[Runner] Error closing browser driver:', err.message);
      }
    }
  ...
  runner.on('fail', (test, err) => {
    ...
    results.push({
      ...
      logs: test.browserLogs || 'No browser console logs captured.',
